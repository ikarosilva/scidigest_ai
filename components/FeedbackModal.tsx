      ...
      <div className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-200">
        <div className="p-6 border-b border-slate-800 flex justify-between items-center">
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-0.5">SciDigest AI v{APP_VERSION}</p>
          </div>
        ...
        <div className="p-6 space-y-6">
          <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800">
            <button 
              type="button"
              onClick={() => setType('bug')}
              className={`flex-1 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${type === 'bug' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
            >
              🐞 Report Bug
            </button>
            <button 
              ...
              className={`flex-1 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${type === 'feature' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
            >
              ✨ Request Feature
            </button>
          </div>
